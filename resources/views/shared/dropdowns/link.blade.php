@if(isset($options['dropdown']['button']))
    <a href="#" class="btn d-flex align-items-center {{ $class }}" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">@include('icons.horizontal_menu', ['class' => 'fill-current width-4 height-4'])&#8203;</a>
@endif
...
<div class="dropdown-menu {{ (__('lang_dir') == 'rtl' ? 'dropdown-menu' : 'dropdown-menu-right') }} border-0 shadow">
    ...
    @if(isset($options['dropdown']['preview']))
        <a class="dropdown-item d-flex align-items-center" href="{{ (($link->domain->name ?? config('app.url')) . '/' . $link->alias) }}/+" target="_blank">@include('icons.preview', ['class' => 'text-muted fill-current width-4 height-4 '.(__('lang_dir') == 'rtl' ? 'ml-3' : 'mr-3')]) {{ __('Preview') }}</a>
    @endif

    @if(isset($options['dropdown']['open']))
        <a class="dropdown-item d-flex align-items-center" href="{{ (($link->domain->name ?? config('app.url')) . '/' . $link->alias) }}" target="_blank" rel="nofollow noreferrer noopener">@include('icons.external', ['class' => 'text-muted fill-current width-4 height-4 '.(__('lang_dir') == 'rtl' ? 'ml-3' : 'mr-3')]) {{ __('Open') }}</a>
    @endif
