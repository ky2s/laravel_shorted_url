                ...
                @foreach($paymentMethods as $payment)
                    <div class="list-group-item px-0">
                        <div class="row align-items-center">
                            <div class="col">
                                <div class="row">
                                    <div class="col-12 col-lg-6 d-inline-flex align-items-center">@include('icons.payments.' . (in_array($payment->card->brand, config('payments')) ? $payment->card->brand : 'unknown'), ['class' => 'width-6']) <span class="mx-2">•••• {{ $payment->card->last4 }}</span> @if($defaultPaymentMethod && $defaultPaymentMethod->id == $payment->id) <span class="badge badge-secondary">{{ __('Default') }}</span>@endif</div>
