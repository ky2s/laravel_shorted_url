    ...
    <div class="card-body">
        ...
        <form action="{{ isset($admin) ? route('admin.spaces.edit', $space->id) : route('spaces.edit', $space->id) }}" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="text" name="name" class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" id="i_name" value="{{ old('name') ?? $space->name }}">
                @if ($errors->has('name'))
                    ...
                    @foreach(formatSpace() as $key => $value)
                        <div class="col-4 col-sm">
                            <div class="custom-control custom-radio custom-control-inline">
                                ...
                                <label class="custom-control-label d-flex align-items-center" for="i_color{{ $key }}"><div class="width-4 height-4 bg-{{ $value }} rounded-circle cursor-pointer"></div></label>
                            </div>
                        </div>
                    @endforeach
                </div>
