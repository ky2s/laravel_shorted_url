...
<div class="card border-0 shadow-sm">
    ...
    <div class="card-body">
        ...
        <div class="form-group">
            <label for="i_cronjob_cache">{!! __(':name command', ['name' => '<span class="badge badge-primary">cache</span>']) !!}</label>
            <div class="input-group">
                <div class="input-group-prepend">
                    <code class="input-group-text" id="basic-addon1">0 6 * * 0</code>
                </div>
                <input type="text" dir="ltr" name="cronjob_cache" id="i_cronjob_cache" class="form-control" value="wget {{ route('cronjob.cache', ['key' => config('settings.cronjob_key')]) }} >/dev/null 2>&1" readonly>
                <div class="input-group-append">
                    <div class="btn btn-primary" data-enable="tooltip-copy" title="{{ __('Copy') }}" data-copy="{{ __('Copy') }}" data-copied="{{ __('Copied') }}" data-clipboard-target="#i_cronjob_cache">{{ __('Copy') }}</div>
                </div>
            </div>
        </div>
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-auto">
                                            @if(request()->input('sort'))
                                                <a href="{{ route('admin.settings.cronjobs') }}" class="text-secondary">{{ __('Reset') }}</a>
                                            @endif
                                        </div>
                                    </div>
                                </div>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content border-0 shadow">
            ...
            <div class="modal-footer">
                ...
                <form action="{{ route('admin.settings.cronjobs') }}" method="post" enctype="multipart/form-data">
