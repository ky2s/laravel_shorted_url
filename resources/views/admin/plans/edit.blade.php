...
<div class="card border-0 shadow-sm">
    ...
    <div class="card-body">
        ...
        <form action="{{ route('admin.plans.edit', $plan->id) }}" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="i_option_pixels">{{ __('Pixels') }}</label>
                <input type="number" name="option_pixels" id="i_option_pixels" class="form-control{{ $errors->has('option_pixels') ? ' is-invalid' : '' }}" value="{{ old('option_pixels') ?? $plan->option_pixels }}">
                @if ($errors->has('option_pixels'))
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $errors->first('option_pixels') }}</strong>
                    </span>
                @endif
            </div>
            ...
            <div class="form-group">
                <label for="i_option_targeting">{{ __('Targeting') }}</label>
                <select name="option_targeting" id="i_option_targeting" class="custom-select{{ $errors->has('option_targeting') ? ' is-invalid' : '' }}">
                    @foreach([1 => __('On'), 0 => __('Off')] as $key => $value)
                        <option value="{{ $key }}" @if((old('option_targeting') !== null && old('option_targeting') == $key) || ($plan->option_targeting == $key && old('option_targeting') == null)) selected @endif>{{ $value }}</option>
                    @endforeach
                </select>
                @if ($errors->has('option_targeting'))
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $errors->first('option_targeting') }}</strong>
                    </span>
                @endif
            </div>
            ...
            <div class="form-group">
                <label for="i_option_data_export">{{ __('Data export') }}</label>
                <select name="option_data_export" id="i_option_data_export" class="custom-select{{ $errors->has('option_data_export') ? ' is-invalid' : '' }}">
                    @foreach([1 => __('On'), 0 => __('Off')] as $key => $value)
                        <option value="{{ $key }}" @if((old('option_data_export') !== null && old('option_data_export') == $key) || ($plan->option_data_export == $key && old('option_data_export') == null)) selected @endif>{{ $value }}</option>
                    @endforeach
                </select>
                @if ($errors->has('option_data_export'))
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $errors->first('option_data_export') }}</strong>
                    </span>
                @endif
            </div>
