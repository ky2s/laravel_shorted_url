...
class RedirectController extends Controller
{
    public function index(Request $request, $id)
    {
        ...
        if ($link) {
            ...
            if ($link->password && session('verified_link') != $link->id) {
                // Cache the referrer
                session(['referrer'.$link->id => $referrer]);
            } elseif($link->password && session('verified_link') == $link->id) {
                // Retrieve the cached referrer
                $referrer = session('referrer'.$link->id);

                // If there's no additional consent required
                if (count($link->pixels) == 0) {
                    // Clear the cached referrer
                    session()->forget('referrer'.$link->id);
                }
            }
            ...
            if (count($link->pixels) > 0) {
                // If the user did not previously visited the consent page
                if (Cookie::get('consent'.$link->id) == null) {
                    // Cache the referrer
                    session(['referrer'.$link->id => $referrer]);
            ...
            if (count($link->pixels) > 0) {
                ...
                if(Cookie::get('consent'.$link->id) == 1) {
